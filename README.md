...
O serviço `backup` corre automaticamente com `docker compose up` - faz um
dump comprimido do Postgres a cada `BACKUP_INTERVAL_HOURS` (24h por
omissão), grava em `./backups/` no teu disco, e apaga backups com mais de
`BACKUP_RETENTION_DAYS` dias (14 por omissão). Como `./backups/` é uma
pasta do teu disco (não um volume Docker), sobrevive a `docker compose
down -v`.

Backup manual, a qualquer momento:
```bash
docker compose exec backup sh -c 'pg_dump -h postgres -U $POSTGRES_USER -d $POSTGRES_DB | gzip > /backups/manual_$(date +%Y%m%d_%H%M%S).sql.gz'
```

Restaurar um backup:
```bash
docker compose exec -it backup sh /scripts/restore.sh evolure_20260830_120000.sql.gz
```
